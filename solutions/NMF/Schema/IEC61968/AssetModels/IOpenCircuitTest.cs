using System.Collections.Specialized;
//------------------------------------------------------------------------------
// <auto-generated>
//     Dieser Code wurde von einem Tool generiert.
//     Laufzeitversion:4.0.30319.42000
//
//     Änderungen an dieser Datei können falsches Verhalten verursachen und gehen verloren, wenn
//     der Code erneut generiert wird.
// </auto-generated>
//------------------------------------------------------------------------------

using NMF.Collections.Generic;
using NMF.Collections.ObjectModel;
using NMF.Expressions;
using NMF.Expressions.Linq;
using NMF.Models;
using NMF.Models.Collections;
using NMF.Models.Expressions;
using NMF.Models.Meta;
using NMF.Models.Repository;
using NMF.Serialization;
using NMF.Utilities;
using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.ComponentModel;
using System.Diagnostics;
using System.Linq;
using TTC2017.SmartGrids.CIM.IEC61968.Metering;
using TTC2017.SmartGrids.CIM.IEC61968.WiresExt;
using TTC2017.SmartGrids.CIM.IEC61970.Core;
using TTC2017.SmartGrids.CIM.IEC61970.Informative.InfAssetModels;
using TTC2017.SmartGrids.CIM.IEC61970.Informative.InfAssets;
using TTC2017.SmartGrids.CIM.IEC61970.Informative.InfERPSupport;
using TTC2017.SmartGrids.CIM.IEC61970.Informative.InfTypeAsset;
using TTC2017.SmartGrids.CIM.IEC61970.Wires;

namespace TTC2017.SmartGrids.CIM.IEC61968.AssetModels
{
    
    
    /// <summary>
    /// The public interface for OpenCircuitTest
    /// </summary>
    [DefaultImplementationTypeAttribute(typeof(OpenCircuitTest))]
    [XmlDefaultImplementationTypeAttribute(typeof(OpenCircuitTest))]
    public interface IOpenCircuitTest : IModelElement, IDistributionWindingTest
    {
        
        /// <summary>
        /// The noLoadLoss property
        /// </summary>
        float NoLoadLoss
        {
            get;
            set;
        }
        
        /// <summary>
        /// The excitingCurrent property
        /// </summary>
        float ExcitingCurrent
        {
            get;
            set;
        }
        
        /// <summary>
        /// The excitingCurrentZero property
        /// </summary>
        float ExcitingCurrentZero
        {
            get;
            set;
        }
        
        /// <summary>
        /// The noLoadLossZero property
        /// </summary>
        float NoLoadLossZero
        {
            get;
            set;
        }
        
        /// <summary>
        /// The MeasuredWindingSpecs property
        /// </summary>
        IOrderedSetExpression<IToWindingSpec> MeasuredWindingSpecs
        {
            get;
        }
        
        /// <summary>
        /// Gets fired before the NoLoadLoss property changes its value
        /// </summary>
        event System.EventHandler<ValueChangedEventArgs> NoLoadLossChanging;
        
        /// <summary>
        /// Gets fired when the NoLoadLoss property changed its value
        /// </summary>
        event System.EventHandler<ValueChangedEventArgs> NoLoadLossChanged;
        
        /// <summary>
        /// Gets fired before the ExcitingCurrent property changes its value
        /// </summary>
        event System.EventHandler<ValueChangedEventArgs> ExcitingCurrentChanging;
        
        /// <summary>
        /// Gets fired when the ExcitingCurrent property changed its value
        /// </summary>
        event System.EventHandler<ValueChangedEventArgs> ExcitingCurrentChanged;
        
        /// <summary>
        /// Gets fired before the ExcitingCurrentZero property changes its value
        /// </summary>
        event System.EventHandler<ValueChangedEventArgs> ExcitingCurrentZeroChanging;
        
        /// <summary>
        /// Gets fired when the ExcitingCurrentZero property changed its value
        /// </summary>
        event System.EventHandler<ValueChangedEventArgs> ExcitingCurrentZeroChanged;
        
        /// <summary>
        /// Gets fired before the NoLoadLossZero property changes its value
        /// </summary>
        event System.EventHandler<ValueChangedEventArgs> NoLoadLossZeroChanging;
        
        /// <summary>
        /// Gets fired when the NoLoadLossZero property changed its value
        /// </summary>
        event System.EventHandler<ValueChangedEventArgs> NoLoadLossZeroChanged;
    }
}

